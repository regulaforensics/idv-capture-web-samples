...
export class App implements OnInit, OnDestroy {
  ...
  isPreparedWithId = signal(false);
  isConnectedToPlatform = signal(false);
  ...
  async startWorkflow() {
    ...
    if (!this.isPreparedWithId()) {
      ...
      if (prepareResult.error) {
        ...
        this.isPreparedWithId.set(false);
        return;
      }
    }
    this.isPreparedWithId.set(true);
  ...
  async ngOnInit() {
    ...
    this.isConnectedToPlatform.set(true);
    const prepareResult = await this.service.prepareWorkflow({
      workflowId: '', // set workflow id
    });
    console.log({ prepareResult });
  ...
  ngOnDestroy() {
    ...
    this.isOpen.set(false);
    this.isPreparedWithId.set(false);
    this.isConnectedToPlatform.set(false);
  }
}
